...
Edited 10/27/2025 by Halle Holzbauer
"""
...
dropdown.select_by_visible_text("Frontal - original test")
...
for x in range(3,19):  #range of page tabs to go through - range(n,m) goes n to m-1 (not m)
...
for idx, case in enumerate(cases):
    ...
    By.XPATH, "//*[contains(translate(text(), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'EXCEL')]"
))).click()
    wait.until(
            ec.presence_of_all_elements_located(
                (By.XPATH, "//*[contains(translate(text(), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'PLOTS') or contains(translate(text(), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'SUMMARY')]")
            )
    )
    time.sleep(2)

    for keyword in ["plots", "Plots", "Summary"]:
        try:
            elements = driver.find_elements(By.PARTIAL_LINK_TEXT, keyword)
            if not elements:
                print(f"No '{keyword}' files for {case_name}")
                continue
            
            print(f"Found {len(elements)} '{keyword}' files.")
            for i, elem in enumerate(elements, start=1):
                try:
                    wait.until(ec.element_to_be_clickable(elem))
                    elem.click()
                    print(f"→ Downloaded {keyword} file {i}/{len(elements)} for {case_name}")
                    time.sleep(2)
                except Exception as e:
                    print(f"Could not click {keyword} link {i}: {e}")
        except Exception as e:
            print(f"Error processing '{keyword}' files for {case_name}: {e}")
