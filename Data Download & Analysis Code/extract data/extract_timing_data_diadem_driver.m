clear
%formatted for data in "diadem\driver" files

files = dir('C:\Users\holzb\Documents\IIHS Research\Data (IIHS)\diadem\driver\*.xls*');
cd 'C:\Users\holzb\Documents\IIHS Research\Data (IIHS)\diadem\driver';

for n = 1:length(files)    
    n
    f = files(n).name; %'CEF1703.xls';
    ...
    head = readtable(f, 'Sheet', find(contains(sheet_names, 'Head_Neck')), 'VariableNamingRule', 'preserve');
    chest = readtable(f, 'Sheet', find(contains(sheet_names, 'Chest')), 'VariableNamingRule', 'preserve');
    ...
    out_file = 'combining test data macro diadem d.xlsm';
%% Vehicle accel data
